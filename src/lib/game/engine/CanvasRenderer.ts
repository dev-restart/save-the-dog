...
import type { BodyLabel, GamePhase, Point, SkinId } from '../types.js';
...
export class CanvasRenderer {
	...
	constructor(skin: SkinId = 'classic') {
	...
	private drawDrawingBody(ctx: CanvasRenderingContext2D, body: Matter.Body): void {
		...
		for (const part of parts) {
			this.drawPolygon(ctx, part, this.skin.drawingFill, this.skin.drawingStroke, 2);
		}
	}
	...
	private drawDrawingPreview(ctx: CanvasRenderingContext2D, points: Point[], inkRatio: number): void {
		...
		ctx.strokeStyle = inkRatio < 0.2 ? '#ef4444' : '#2563eb';
		ctx.lineWidth = 6;
		ctx.lineCap = 'round';
		ctx.lineJoin = 'round';
		ctx.stroke();
